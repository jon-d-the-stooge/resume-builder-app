/**
 * Test the new holistic resume optimization approach
 *
 * This uses a single-pass holistic analysis instead of element-by-element matching.
 */

import { LLMClient } from './src/shared/llm/client';
import { optimizeResume, analyzeOnly } from './src/ats-agent/holistic';
import dotenv from 'dotenv';

dotenv.config();
process.env.LLM_DEBUG = '1';

async function testHolistic() {
...
• Innovation and Discovery Award, 2018
  ...
  console.log('=== HOLISTIC ANALYSIS TEST ===\n');

  // First, just run analysis to see the score
  console.log('--- Running Analysis Only ---');
  const analysis = await analyzeOnly(job, resume, llmClient);

  console.log(`\n=== ANALYSIS RESULTS ===`);
  console.log(`Fit Score: ${(analysis.overallFit * 100).toFixed(1)}%`);
  console.log(`Assessment: ${analysis.fitAssessment}`);

  console.log(`\n--- Semantic Connections (${analysis.connections.length}) ---`);
  analysis.connections.forEach((conn, i) => {
    console.log(`${i + 1}. [${conn.connectionType.toUpperCase()}] ${conn.jobRequirement}`);
    console.log(`   Evidence: ${conn.resumeEvidence}`);
    console.log(`   Confidence: ${(conn.confidence * 100).toFixed(0)}%`);
  });
  ...
  console.log(`\n--- Gaps ---`);
  analysis.gaps.forEach((g, i) => console.log(`${i + 1}. ${g}`));

  console.log(`\n--- Top Recommendations ---`);
  analysis.recommendations.slice(0, 5).forEach((rec, i) => {
    console.log(`${i + 1}. [${rec.priority.toUpperCase()}] ${rec.jobRequirementAddressed}`);
    console.log(`   Current: "${rec.currentContent.substring(0, 80)}..."`);
    console.log(`   Suggested: "${rec.suggestedReframe.substring(0, 80)}..."`);
  });

  console.log(`\n--- Terminology Alignments ---`);
  analysis.terminologyAlignments.forEach((t, i) => {
    console.log(`${i + 1}. "${t.resumeTerm}" → "${t.jobTerm}"`);
  });

  console.log(`\n--- Themes Identified ---`);
  console.log(analysis.themesIdentified.join(', '));

  // Now run full optimization with rewriting
  console.log('\n\n=== RUNNING FULL OPTIMIZATION ===\n');

  const result = await optimizeResume(job, resume, llmClient, {
    targetFit: 0.85,
    maxIterations: 2,
  });

  console.log(`\n=== OPTIMIZATION COMPLETE ===`);
  console.log(`Initial Fit: ${(result.initialFit * 100).toFixed(1)}%`);
  console.log(`Final Fit: ${(result.finalFit * 100).toFixed(1)}%`);
  console.log(`Improvement: +${(result.improvement * 100).toFixed(1)}%`);
  console.log(`Iterations: ${result.iterations.length}`);
  console.log(`Termination: ${result.terminationReason}`);

  if (result.iterations.length > 1 && result.iterations[0].rewrite) {
    console.log(`\n--- Changes Applied ---`);
    result.iterations[0].rewrite.changesApplied.forEach((c, i) => {
      console.log(`${i + 1}. ${c}`);
    });
  }
}

testHolistic().catch(console.error);
